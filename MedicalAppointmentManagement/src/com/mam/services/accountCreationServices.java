...
public class accountCreationServices {
	...
	private keyGenerator randomizeID = new keyGenerator();
	
	
	public boolean accountCreate(String firstName, String lastName, String email, String password) throws mamException, mamThrowableException{
		...
		if(accValid.validateAll(accObj)){		//Call for validation
			...
			//do{
				...
				int Id = randomizeID.randomizeAccID();	//generate random 6 digit number
				...
				if (!accCreate.checkAccID(accObj.getAccID())){	//check generated id with database for duplicate
					if(!accCreate.checkEmailDuplicate(accObj.getEmail())){	//check email for duplicate
							...
							try {
								...
								throw new mamThrowableException("SQLException ERROR in CreationService:- ",e);
